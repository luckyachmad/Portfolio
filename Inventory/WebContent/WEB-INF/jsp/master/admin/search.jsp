...
<body>
	...
	<div id="container-full">
		...
		<div id="main-full">
				...
				<table class="second">
					...
					<c:forEach var="customer" items="${customer}" varStatus="status">
						<tr>
							...
							<td><a class="view" href="<c:url value="/master/admin/detail?id=${customer.id}"/>">${customer.id}</a></td>
							<td>${customer.name}</td>
							<td>${customer.alamat}</td>
							<td>${customer.email}</td>
							<td>
								<c:if test="${customer.enable}">Aktif</c:if>
								<c:if test="${not empty customer.enable && !customer.enable}">Tidak Aktif</c:if>
							</td>
						</tr>
					</c:forEach>
			...
			<a class="button" href="<c:url value="/master/admin/detail"/>">Create Customer</a>
			<br/><br/>
